...
- `highway.getStringCount()` — number of strings on the active arrangement (4 for bass, 6 for guitar, 7+ for extended-range GP imports). Derived server-side as `max(notes-max-string + 1, name-based fallback)` where the fallback is 4 for arrangements containing "bass" (case-insensitive) and 6 otherwise. The combination handles partial-string-usage charts (a 6-string lead that never plays string 5) and extended-range GP imports (5-string bass, 7-string guitar) without requiring plugins to do their own arrangement-name matching
