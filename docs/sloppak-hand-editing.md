...
3. Note the **sample rate** displayed in Audacity's status bar (typically `44100 Hz` for a converted PSARC). Your recording must match this.
...
- **Authoring a sloppak from scratch** (no source PSARC, no Guitar Pro file) — that's a developer task. Start at [sloppak-spec.md §4.2](sloppak-spec.md#42-writing-python-server-side).
